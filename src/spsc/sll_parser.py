...
fRule.setParseAction(lambda t: FRule(t[0], t[1], t[2]))

gParamList = Group(Optional(Suppress(",") + delimitedList(lIdent)))

gRule = gIdent + LPAR + pattern + gParamList + RPAR + EQ + exp + SC
gRule.setParseAction(lambda t: GRule(t[0], t[1], t[2], t[3], t[4]))

program = ZeroOrMore(fRule | gRule)
program.setParseAction(lambda t: Program(t))
